...
public class SanPhamCTService implements SanPhamCTImpl{
    ...
    public List<SanPhamCT> getAll() {
        String sql = "SELECT SanPhamCT.idSP,SanPhamCT.maSP,tenSP,tenThuongHieu,LoaiSP,tenSize,tenChatLieu,tenMauSac,SanPhamCT.giaNhap,LichSuGia.gia,soLuong, ngayNhap, trangThai \n"
...
//                + "WHERE SanPhamCT.trangThai = N'Hoạt động'";
    ...
    public void addSanPhamCT(SanPhamCT spct) {
            ...
            String sql1 = "Insert into SanPhamCT(maSP,loaiSP,soLuong,giaNhap,trangThai,idMauSac,idSize,idThuongHieu,idChatLieu)" + "Values(?,?,?,?,?,?,?,?,?)";
            Connection conn1 = DBconnect.getConnection();
            PreparedStatement psm1 = conn1.prepareStatement(sql1);
            psm1.setString(1, spct.getMaSP());
            psm1.setString(2, spct.getLoaiSP());
            psm1.setInt(3, spct.getSoLuong());
            psm1.setDouble(4, spct.getGiaNhap());
            psm1.setString(5, spct.getTrangThai());
            psm1.setInt(6, spct.getIdMauSac());
            psm1.setInt(7, spct.getIdSize());
            psm1.setInt(8, spct.getIdThuongHieu());
            psm1.setInt(9, spct.getIdChatLieu());
            psm1.executeUpdate();
            psm1.close();
